...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      // Center the text in the offscreen canvas vertically, adjust left for negative bounds
      ...
      const xOffset = isDesktop ? 0 : (width / 2 - offscreen.width / 2); 
      targets.forEach(t => t.x = xOffset + t.x);

      // Keep max particles reasonable to prevent severe lag
      const maxParticles = Math.max(900, Math.min(6000, Math.floor((width * height) / 60)));
      const stride = Math.max(1, Math.ceil(targets.length / maxParticles));
      const baseRgb = hexToRgb(color);
      const highlightRgb = hexToRgb(highlightColor);
      const selected = targets.filter((_, index) => index % stride === 0);

      const isMorphing = particles.length > 0;
      const newParticles = [];
      ...
      selected.forEach((target, index) => {
        ...
        const size = Math.max(0.6, particleSize * (0.75 + target.alpha * 0.45));

        if (isMorphing && index < particles.length) {
          // Recycle existing particle
          const p = particles[index];
          p.targetX = target.x;
          p.targetY = target.y;
          p.color = particleColor; // Update color just in case
          p.size = size;
          newParticles.push(p);
        } else {
          // Create new particle
          let startX, startY;
          if (isMorphing && particles.length > 0) {
            // Morphing but we need MORE particles: spawn them from a random existing particle
            ...
            startX = target.x + Math.cos(angle) * distance + (seed - 0.5) * scatter * 0.45;
            startY = target.y + Math.sin(angle) * distance + (depth - 0.9) * scatter * 0.45;
          }
